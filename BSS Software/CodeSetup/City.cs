...
namespace BSSSoftware.CodeSetup
{
    public partial class City : Form
    {
        ...
        #region Variables
        private CityControl m_controller = null;
        string key = null;
        #endregion
        ...
        {
            m_controller = new CityControl();
            GridViewDataBind();
            cboDivisionBind();
        }
        public void GridViewDataBind()
        {
            dgvCity.DataSource = m_controller.SelectAll();
            dgvCity.AutoGenerateColumns = false;
        }
        private void cboDivisionBind()
        {
            DivisionControl d_control = new DivisionControl();
            cbodivision.DataSource = d_control.SelectAll();
            cbodivision.DisplayMember = "Division";
            cbodivision.ValueMember = "DivId";
        }

        public void Save(string key)
        {
            if (txtCity.Text.Equals(null)) return;
            xsdCodeSetup.CityRow dataRow = (new xsdCodeSetup.CityDataTable()).NewCityRow();
            try
            {

                dataRow.City = txtCity.Text.Trim();
        ...
        {
            if ((this.dgvCity.SelectedRows == null) || (this.dgvCity.SelectedRows.Count < 1)) return;

            DataRowView dataRowView = this.dgvCity.SelectedRows[0].DataBoundItem as DataRowView;
            if (dataRowView != null)
            {
                string key = Global.GetDataFromRow<string>(dataRowView.Row, "CityId", string.Empty);
                this.txtCity.Text = Global.GetDataFromRow<string>(dataRowView.Row, "City", string.Empty);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (dataRowView != null)
            {
                key = Global.GetDataFromRow<string>(dataRowView.Row, "CityId", string.Empty);
            }
            this.Delete(key);
            GridViewDataBind();
        }
