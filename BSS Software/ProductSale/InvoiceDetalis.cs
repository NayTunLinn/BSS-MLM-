...
{
    public partial class ReturnInvoiceDetails : Form
    {
        ...
        public void DataBind()
        {
            ...
            dgvInvoiceDetails.DataSource = s_controller.InvoiceDetailsSelectByMonth(DateTime.Now);

        }

        private void dgvInvoiceDetails_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1) return;
            if (e.ColumnIndex == -1) return;


            string InvId = dgvInvoiceDetails.Rows[e.RowIndex].Cells["colInvId"].Value.ToString();
        ...
        private void InvoiceDataBind() ///for autocomplete Inv No
        {
        ...
        {
            dgvInvoiceDetails.AutoGenerateColumns = false;
            dgvInvoiceDetails.DataSource = s_controller.InvoiceDetailsSelectTotal(PostSql);            
        }
        ...
        private void GeneralSearch()
        {
            ...
            if (!txtinvoiceno.Text.Equals(string.Empty))
            {
                if (isFirst)
                {
                    PostSql += " WHERE dbo.Invoice.InvNo LIKE " + "'%" + txtinvoiceno.Text.Trim() + "%'";
                    isFirst = false;
                }
                else
                {
                    PostSql += " AND dbo.Invoice.InvNo LIKE LIKE " + "'%" + txtinvoiceno.Text.Trim() + "%'";
                }
            }
            if (!txtMerchantCode.Text.Equals(string.Empty))
            {
                if (isFirst)
                {
                    PostSql += " WHERE dbo.Invoice.UpperdistCode LIKE " + "'%" + txtMerchantCode.Text + "%'";
                    isFirst = false;
                }
                else PostSql += " AND dbo.Invoice.UpperdistCode LIKE " + "'%" + txtMerchantCode.Text + "%'";
        ...
        {
            Reporting.InvoiceDetailReport invDetail = new Reporting.InvoiceDetailReport(PostSql);
            invDetail.Show();
            this.Close();
        }
